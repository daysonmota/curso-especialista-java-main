    ...
    public void adicionarItem(ItemVenda item) {
        if (excedeLimiteDeCompra(item)) {
            throw new LimiteDeCompraExcedidoException(
                    String.format("Limite atual de compras do cliente é: %.2f. Você só tem : %.2f disponível no carrinho ", cliente.getLimiteCompras(), cliente.getLimiteCompras() -  valorTotal));
        }
